...
fn sha512(W: ptr<function, array<u32, 32>>, IV: ptr<function, array<u32, 16>>) {
  ...
  for (var i: u32 = 0u; i < 32u; i = i + 2u) {
      ...
      var t2_lo = tmp + ((alo & blo) ^ (alo & clo) ^ (blo & clo));
      var t2_hi = (((ahi >> 28) | (alo << 4)) ^ ((alo >> 2) | (ahi << 30)) ^ ((alo >> 7) | (ahi << 25))) + ((ahi & bhi) ^ (ahi & chi) ^ (bhi & chi)) + select(0u, 1u, t2_lo < tmp);
      dlo += t1_lo;
  ...
  for (var i: u32 = 32u; i < 160u; i = i + 2u) {
      var xhi = W[(i + 28) & 0x1f];
      var xlo = W[(i + 29) & 0x1f];
      var tx1_hi = ((xhi >> 19) | (xlo << 13)) ^ ((xlo >> 29) | (xhi << 3)) ^ (xhi >> 6);
      var tx1_lo = ((xlo >> 19) | (xhi << 13)) ^ ((xhi >> 29) | (xlo << 3)) ^ ((xlo >> 6) | (xhi << 26));
      xhi = W[(i + 2) & 0x1f];
      xlo = W[(i + 3) & 0x1f];
      var tx3_hi = ((xhi >> 1) | (xlo << 31)) ^ ((xhi >> 8) | (xlo << 24)) ^ (xhi >> 7);
      var tx3_lo = ((xlo >> 1) | (xhi << 31)) ^ ((xlo >> 8) | (xhi << 24)) ^ ((xlo >> 7) | (xhi << 25));
      ...
      // t1_lo = glo + (((dlo >> 14) | (dhi << 18)) ^ ((dlo >> 18) | (dhi << 14)) ^ ((dhi >> 9) | (dlo << 23)));
      // t1_hi = ghi + (((dhi >> 14) | (dlo << 18)) ^ ((dhi >> 18) | (dlo << 14)) ^ ((dlo >> 9) | (dhi << 23))) + select(0u, 1u, t1_lo < hlo);
      // tmp = (dlo & elo) ^ ((~dlo) & flo);
      // t1_lo = t1_lo + tmp;
      // t1_hi = t1_hi + ((dhi & ehi) ^ ((~dhi) & fhi)) + select(0u, 1u, t1_lo < tmp);
      // tmp = K[i + 1];
      ...
      // glo = elo;
      // flo = dlo + t1_lo;
      // fhi = dhi + t1_hi + select(0u, 1u, flo < t1_lo);
      // ehi = chi;
      // elo = clo;
      // dhi = bhi;
      // dlo = blo;
